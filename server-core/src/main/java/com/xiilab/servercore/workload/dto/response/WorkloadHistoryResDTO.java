	...
	public static class FindWorkload extends WorkloadHistoryResDTO {
		...
		public static FindWorkload from(JobEntity jobEntity) {
			return FindWorkload.builder()
				...
				.image(new FindWorkload.Image(jobEntity.getImageWorkloadMappingEntity().getImage()))
				.ports(jobEntity.getPortList().stream().map(FindWorkload.Port::new).toList())
				.envs(jobEntity.getEnvList().stream().map(FindWorkload.Env::new).toList())
				.datasets(jobEntity.getDatasetWorkloadMappingList().stream().map(FindWorkload.Volume::new).toList())
				.models(jobEntity.getModelWorkloadMappingList().stream().map(FindWorkload.Volume::new).toList())
				.codes(jobEntity.getCodeWorkloadMappingList().stream().map(FindWorkload.Code::new).toList())
				.command(jobEntity.getWorkloadCMD())
				.cpuRequest(jobEntity.getCpuRequest())
				.gpuRequest(jobEntity.getGpuRequest().floatValue())
				.memRequest(jobEntity.getMemRequest())
