...
public enum Promql {
	...
	// REPORT_SYSTEM_INFO_GPU("label_replace(DCGM_FI_DEV_FAN_SPEED, \"internal_ip\",\"$1\",\"instance\",\"(.*):(.*)\")", "", "REPORT"),
	...
	REPORT_SYSTEM_AVG_TEMP("avg(DCGM_FI_DEV_GPU_TEMP) by(kubernetes_node, gpu)","",""),
	REPORT_SYSTEM_MAX_TEMP("max(DCGM_FI_DEV_GPU_TEMP) by(kubernetes_node, gpu)","",""),
	...
	REPORT_SYSTEM_DISK_USAGE("label_replace((node_filesystem_size_bytes{mountpoint=\"/\"} - node_filesystem_avail_bytes{mountpoint=\"/\"}) / (node_filesystem_size_bytes{mountpoint=\"/\"}) * 100,  \"internal_ip\", \"$1\", \"instance\", \"(.*):(.*)\") * on (internal_ip) group_left(node) kube_node_info{} > 90","",""),
